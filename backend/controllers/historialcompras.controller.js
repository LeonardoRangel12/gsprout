...
const createHistorialCompra = async (req, res) => {
  try {
    ...
    const historialCompra = await historialComprasService.createHistorialCompra(
      value
    );
...
const getHistorialCompraById = async (req, res, next) => {
  try {
    const historialCompra =
      await historialComprasService.getHistorialCompraById(req.params.id);
    if (!historialCompra)
      return res.status(404).send("Historial de compra no existe");
...
const updateHistorialCompra = async (req, res) => {
  try {
    ...
    const historialCompra = await historialComprasService.updateHistorialCompra(
      req.params.id,
      value
    );
    return res.status(200).send(historialCompra);
